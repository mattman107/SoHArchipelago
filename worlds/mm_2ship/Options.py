...
class HintsGossipStoneStrength(Range):
    """
    How strongly gossip stone hints are weighted toward useful, unobtained checks
    rather than a uniformly random shuffled check. 0 is fully random, 100 always
    picks from the most useful remaining checks.
    """
...
class Logic(Choice):
    """
    Glitchless: full reachability logic, generated from the 2ship randomizer's
    own region/logic definitions (including the three-day time system).
    Vanilla: same rules as glitchless.
    Nearly No Logic / No Logic: no access rules; anything can be anywhere.
    """
    display_name = "Logic"
    option_glitchless = 0
    option_vanilla = 1
...
class PlacementSmallKeys(Choice):
    """
    Anywhere: Small Keys can be placed at any location.
    Own Dungeon: Each dungeon's Small Keys are confined to locations within that same dungeon.
    """
...
class PlacementBossKeys(Choice):
    """
    Anywhere: Boss Keys can be placed at any location.
    Own Dungeon: Each dungeon's Boss Key is confined to locations within that same dungeon.
    """
...
class PlacementStrayFairies(Choice):
    """
    Anywhere: Stray Fairies can be placed at any location.
    Own Dungeon: Each dungeon's Stray Fairies are confined to locations within that same dungeon.
    """
...
class ShuffleSkeletonKey(Toggle):
    ...
    number of Small Keys for every dungeon (Woodfall 1, Snowhead 3, Great Bay 1,
    Stone Tower 4), in addition to each dungeon's own Small Keys already in the pool.
    """
...
class ShuffleTycoonWallet(Toggle):
    """
    Add a third wallet upgrade into the pool (5000 rupee capacity), on top of the
    Adult's and Giant's Wallet upgrades already in the pool.
    """
