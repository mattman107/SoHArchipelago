...
class MM2ShipState(LogicMixin):
    ...
    The solver recomputes the full region/time/event fixpoint only when the
    state's inventory changed (collect/remove pop the cached result); identical
    inventories across states share results through the world-level memo.
    """
...
class MM2ShipWorld(World):
    ...
    def use_logic(self) -> bool:
        ...
        if self.options.true_no_logic.value:
            return False
        return self.options.logic.value in (0, 1)  # glitchless, vanilla
    ...
    def generate_early(self) -> None:
        ...
        if self.options.clock_shuffle.value:
            if self.options.clock_shuffle_progressive.value == 0:  # RO_CLOCK_SHUFFLE_RANDOM
    ...
    def fill_slot_data(self) -> dict[str, Any]:
        slot_data = {
            ...
            "hints_transformations": self.options.hints_transformations.value,
            "hints_gossip_stone_strength": self.options.hints_gossip_stone_strength.value,
            # Logic
            "logic": self.options.logic.value,
            # Dungeon item placement
            "placement_small_keys": self.options.placement_small_keys.value,
            "placement_boss_keys": self.options.placement_boss_keys.value,
            "placement_stray_fairies": self.options.placement_stray_fairies.value,
            ...
            "shuffle_skeleton_key": self.options.shuffle_skeleton_key.value,
